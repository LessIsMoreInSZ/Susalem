...
// #region 存储页面条数
export const setPageSize = (pageKey: string, pageSize: any) => {
  if (typeof pageSize === "number") {
    pageSize = pageSize.toString()
    localStorage.setItem(pageKey, pageSize)
  }
}
export const getPageSize = (pageKey: string) => {
  const currentPageSize = localStorage.getItem(pageKey)!
  return parseInt(currentPageSize)
}
/**清空所有页面存储的条数 */
export const removeAllPageSize = (allPageSize: Array<string>) => {
  console.log("%c [ allPageSize ]-75", "font-size:13px; background:pink; color:#bf2c9f;", allPageSize)
  allPageSize.forEach((pageKey) => {
